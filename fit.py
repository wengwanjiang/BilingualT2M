...
for path in paths:
    ...
    # HDF5 may use extended float types h5py cannot map; cast explicitly.
    init_mean_pose = (
        torch.from_numpy(file["pose"].astype(np.float64)[:])
        .unsqueeze(0).repeat(joints.shape[0], 1)
        .float()
        .to(device)
    )
    init_mean_shape = (
        torch.from_numpy(file["shape"].astype(np.float64)[:])
        .unsqueeze(0).repeat(joints.shape[0], 1)
        .float()
        .to(device)
    )
